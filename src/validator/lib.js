 ...
 * @typedef {{ fromStore?: DelegationStoreGetter }} StoreGetterConfig
 */
...
/**
 * @param {Required<API.ClaimOptions & StoreGetterConfig>} config
...
const resolveSources = async ({ delegation }, config) => {
	...
	for (const proof of allDelegations) {
 ...
 * @param {API.ValidationOptions<API.ParsedCapability<A, R, C>> & StoreGetterConfig} options
 * @returns {Promise<API.Result<API.Authorization<API.ParsedCapability<A, R, C>>, API.Unauthorized>>}
 */
export const access = async (invocation, { capability, ...config }) => claim(capability, [invocation], config);
 ...
 * @param {API.Proof[]} proofs
 * @param {API.ClaimOptions & StoreGetterConfig} config
 * @returns {Promise<API.Result<API.Authorization<API.ParsedCapability<A, R, C>>, API.Unauthorized>>}
 */
...
class InvalidClaim extends Failure {
	...
	describe() {
		...
		return [
			...
			...(unknown.length > 0 ? [li(`Encountered unknown capabilities\n${unknown.join('\n')}`)] : []),
		].join('\n');
	}
}
